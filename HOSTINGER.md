# Déploiement sur Hostinger (VPS) — N'KULU
...
```bash
git clone VOTRE_DEPOT nkulu && cd nkulu
...
```bash
cp deploy/nginx.conf /etc/nginx/sites-available/nkulu
sed -i 's/VOTRE-DOMAINE/votre-domaine.fr/g' /etc/nginx/sites-available/nkulu
ln -s /etc/nginx/sites-available/nkulu /etc/nginx/sites-enabled/nkulu
...
```bash
# Dump quotidien à 3h
0 3 * * * docker compose -f /root/nkulu/docker-compose.yml exec -T db pg_dump -U nkulu nkulu > /root/backups/nkulu-$(date +\%F).sql
```

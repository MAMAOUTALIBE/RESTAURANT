...
const RESERVATION_FIELDS = ["name", "phone", "email", "date", "time", "guests", "notes"];
...
/** Menu réel (plats disponibles) servant à la fois au prompt et à la validation des actions. */
...
async function getHoursText(): Promise<string> {
  const hours = await prisma.openingHour
    ...
    .catch(() => []);
  if (!hours.length) return "Horaires indicatifs : tous les jours de 11h00 à 23h00.";
  return hours
    .map((h) =>
      h.closed
...
export async function buildSystemPrompt(
  ...
  // Menu groupé par catégorie (pour suggérer un repas entrée + plat + boisson).
  let menuText = "Menu indisponible ; invite l'utilisateur à consulter la page Menu.";
  if (menu.length) {
...
- Conseiller des plats, composer un repas complet (entrée + plat + boisson/dessert) et ajouter les plats au panier.
- Renseigner sur les horaires, la livraison (codes postaux desservis), les réservations et le traiteur.
...
RÈGLES :
- N'invente jamais un plat, un slug, un prix, un horaire ou une zone de livraison.
  ...
  for (const raw of rawActions.slice(0, 8)) {
    ...
    if (a.type === "open_page" && typeof a.page === "string") {
      ...
      if (page && !seen.has(`page:${pageKey}`)) {
        ...
        const href =
          pageKey === "reservation" && a.prefill && typeof a.prefill === "object"
...
export function ruleBasedAnswer(input: string): string {
  ...
  if (q.includes("menu") || q.includes("plat") || q.includes("prix") || q.includes("poulet")) {
    return "Le menu contient les plats disponibles avec prix, options et ajout au panier. Rendez-vous sur la page Menu.";
  }

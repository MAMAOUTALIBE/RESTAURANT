  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <h1 className="font-display text-3xl font-bold text-cream">Livraisons</h1>
        <span className="text-sm text-muted">
          {orders.length} à livrer · {enCours} en cours · {drivers.length} livreur·s actifs
        </span>
      </div>
      ...
      {orders.length === 0 ? (
        ...
        <ul className="space-y-3">
          {orders.map((o) => (
            <li key={o.id} className="rounded-2xl border border-white/10 bg-ink-soft p-5">
              <div className="flex flex-wrap items-center justify-between gap-3">
                <Link href={`/admin/commandes/${o.reference}`} className="font-mono text-gold hover:underline">
                  {o.reference}
                </Link>
                <span className={`rounded-full px-2.5 py-1 text-xs font-semibold ${statusStyles[o.status] ?? "bg-white/10 text-cream"}`}>
                  {o.status}
                </span>
                <span className="text-sm text-cream">{formatPrice(o.total)}</span>
              </div>
              <p className="mt-2 text-sm text-cream/85">{o.customerName} · {o.customerPhone}</p>
              {o.customerAddress && <p className="text-sm text-muted">{o.customerAddress}</p>}

              <form action={adminAssignDriver} className="mt-3 flex flex-wrap items-center gap-2">
                <input type="hidden" name="reference" value={o.reference} />
                <label htmlFor={`d-${o.id}`} className="text-sm text-cream/80">Livreur :</label>
                <select
                  id={`d-${o.id}`}
                  name="driverId"
                ...
                {o.driver && (
                  <span className="text-xs text-muted">Actuel : {o.driver.name}</span>
                )}
              </form>
            </li>
          ))}

...
export default async function RecapPage({
  ...
  const byMode = { livraison: 0, emporter: 0, surplace: 0 } as Record<string, number>;
  for (const o of valid) byMode[o.fulfillment] = (byMode[o.fulfillment] ?? 0) + 1;
  ...
  for (const o of valid) {
    const ev = [...o.events].sort((a, b) => a.createdAt.getTime() - b.createdAt.getTime());
    const startPrep = ev.find((e) => e.status === "en préparation");
    const ready = ev.find((e) => e.status === "prête");
    if (startPrep && ready) {
      durations.push((ready.createdAt.getTime() - startPrep.createdAt.getTime()) / 60000);
    }
  }
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <h1 className="font-display text-3xl font-bold text-cream">Récap de service</h1>
        <form action="/admin/recap" className="flex items-center gap-2">
          <a href={`/admin/recap?date=${iso(prevDay)}`} className="rounded-lg border border-white/10 px-3 py-1.5 text-sm text-cream hover:border-gold/60">←</a>
          <input
            type="date"
            name="date"
          ...
          />
          <button className="rounded-lg bg-gold px-3 py-1.5 text-sm font-semibold text-ink">OK</button>
          <a href={`/admin/recap?date=${iso(nextDay)}`} className="rounded-lg border border-white/10 px-3 py-1.5 text-sm text-cream hover:border-gold/60">→</a>
        </form>
      </div>
      ...
      <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
        <Kpi label="Commandes" value={String(valid.length)} sub={cancelled ? `${cancelled} annulée(s)` : undefined} />
        <Kpi label="CA du jour" value={formatPrice(ca)} highlight />
        <Kpi label="Panier moyen" value={formatPrice(avgBasket)} />
        <Kpi label="Prépa moyenne" value={durations.length ? `${avgPrep.toFixed(0)} min` : "—"} sub={`${durations.length} mesurée(s)`} />
      </div>

      <div className="grid gap-6 lg:grid-cols-2">
        {/* Répartition par mode */}
        <section className="rounded-2xl border border-white/10 bg-ink-soft p-6">
          <h2 className="font-display text-lg font-semibold text-cream">Par mode</h2>
          <ul className="mt-4 space-y-2 text-sm">
            <Mode label="🛵 Livraison" n={byMode.livraison} total={valid.length} />
            <Mode label="🛍️ À emporter" n={byMode.emporter} total={valid.length} />
            <Mode label="🍽️ Sur place" n={byMode.surplace} total={valid.length} />
          </ul>
        </section>

        {/* Top plats */}
        <section className="rounded-2xl border border-white/10 bg-ink-soft p-6">
          <h2 className="font-display text-lg font-semibold text-cream">Top plats du jour</h2>
          {topDishes.length === 0 ? (
...
function Mode({ label, n, total }: { label: string; n: number; total: number }) {

    ...
    <div className="space-y-8">
      ...
      <div className="grid gap-4 sm:grid-cols-2">
        <div className="rounded-2xl border border-white/10 bg-ink-soft p-5">
          ...
          <p className="mt-1 font-display text-3xl font-bold text-cream">{count}</p>
          <a
      ...
      <section className="rounded-2xl border border-white/10 bg-ink-soft p-6">
        ...
        <form action={sendCampaign} className="mt-4 space-y-3">
          <div>
            <label htmlFor="audience" className="mb-1.5 block text-sm text-cream/80">
              Audience
            </label>
            <select
              id="audience"
      ...
      {/* Codes promo */}
      <section className="rounded-2xl border border-white/10 bg-ink-soft p-6">
        <h2 className="font-display text-lg font-semibold text-cream">Codes promo</h2>

        <form action={adminCreatePromo} className="mt-4 flex flex-wrap items-end gap-3">
          <input name="code" placeholder="CODE" required className="rounded-lg border border-white/10 bg-ink px-3 py-2 text-sm uppercase text-cream focus:border-gold/60 focus:outline-none" />
          <select name="type" className="rounded-lg border border-white/10 bg-ink px-3 py-2 text-sm text-cream focus:border-gold/60 focus:outline-none">
            <option value="percent">% remise</option>
            <option value="fixed">€ remise</option>
          </select>
          <input name="value" type="number" step="0.5" placeholder="Valeur" required className="w-24 rounded-lg border border-white/10 bg-ink px-3 py-2 text-sm text-cream focus:border-gold/60 focus:outline-none" />
          <input name="usageLimit" type="number" placeholder="Limite (∞)" className="w-28 rounded-lg border border-white/10 bg-ink px-3 py-2 text-sm text-cream focus:border-gold/60 focus:outline-none" />
          <button type="submit" className="rounded-lg bg-gold px-4 py-2 text-sm font-semibold text-ink transition hover:bg-gold-400">
            Créer
          </button>
        </form>
          ...
          <ul className="mt-5 divide-y divide-white/10">
            {promos.map((p) => (
              <li key={p.id} className="flex flex-wrap items-center gap-3 py-3 text-sm">
                ...
                <span className={`rounded-full px-2 py-0.5 text-xs ${p.active ? "bg-green-500/15 text-green-300" : "bg-white/10 text-muted"}`}>
                ...
                <div className="ml-auto flex items-center gap-2">
                  <form action={adminTogglePromo}>
                    ...
                    <button type="submit" className="text-xs text-cream/70 hover:text-gold">

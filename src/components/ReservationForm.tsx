  ...
  return (
    <form action={formAction} className="space-y-4">
      ...
      <div className="grid gap-4 sm:grid-cols-2">
        <Field id="name" label="Nom complet" error={state?.errors?.name} />
      ...
      <div className="grid gap-4 sm:grid-cols-3">
        <Field id="date" label="Date" type="date" error={state?.errors?.date} />
        <Field id="time" label="Heure" type="time" error={state?.errors?.time} />
        <Field id="guests" label="Convives" type="number" error={state?.errors?.guests} />
      </div>
      <div>

...
export function ReviewForm() {
  const [state, formAction] = useFormState<ActionState | null, FormData>(submitReview, null);
  ...
  return (
    <form action={formAction} className="space-y-4">
        ...
        <div className="flex gap-1">
          {[1, 2, 3, 4, 5].map((n) => (
            <button key={n} type="button" onClick={() => setRating(n)} aria-label={`${n} étoile${n > 1 ? "s" : ""}`}>
              <Star className={`h-7 w-7 ${n <= rating ? "fill-gold text-gold" : "text-white/20"}`} />
            </button>
          ))}
        </div>
      </div>
      <div>
        <label htmlFor="r-name" className="mb-1.5 block text-sm text-cream/80">Nom</label>
      ...
      <div>
        <label htmlFor="r-comment" className="mb-1.5 block text-sm text-cream/80">Votre avis</label>
        <textarea id="r-comment" name="comment" rows={3} required className={field} />
        {state?.errors?.comment && <p className="mt-1 text-xs text-red-400">{state.errors.comment}</p>}
      </div>
      {state && !state.ok && !state.errors && <p role="alert" className="text-sm text-red-400">{state.message}</p>}
      <SubmitButton />
    </form>
  );
}

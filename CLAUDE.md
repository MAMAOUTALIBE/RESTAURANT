...
Full-stack restaurant website (showcase + online ordering + admin CRM) built with **Next.js 16 (App Router)**, TypeScript (strict), PostgreSQL via **Prisma 6**, Zod, Tailwind 3, and Framer Motion. The codebase, UI, comments, commit messages, and order-status strings are all in **French** — match that language when editing.
...
npm run db:seed      # tsx prisma/seed.ts (categories, dishes, options, hours, restaurant…)
